...
We also need to set up the p parameter, and do some small preparations.
...
We now build the full reconstruction using the selected models. This code takes about 30 seconds (depending on your computer), and will give you reconstruction results for all 30 available stations. To get results for all 48 stations, skip to the next chunk to read the pre-computed results.
...
ldsFit <- 
  foreach(s = stationIDs, 
    ...
    u <- t(ensemblePCs[[s]][[kwfMax]][[p]])
    transform = trans[s, trans]
    LDS_reconstruction(Qa, u, u, 1200, transform = transform, num.restarts = 50)
}
```
...
We can also create the full 30-member ensembles using the following code, which takes about 15 minutes. Again, we provided pre-computed results for all 48 stations.
...
# saveRDS(ensembleRec, 'results/ensembleRec.RDS') # Use RDS format as it is compressed
```
